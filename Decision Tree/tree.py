...
class DecisionTree:
    ...
    def grow_tree(self, X, y, depth=0):
        n_samples, n_feates = X.shape
        n_labels = len(np.unique(y))

        #1. checking the stopping criteria 
    ...
    #Information Gain(IG) = Entropy(parent) - [weighted average] x Entropy(children)
    ...
    # Entropy(E) = -(prob(yes) * log prob(yes) - prob(no) * log prob(no))
    def _entropy(self, y):
        hist = np.bincount(y)
        ps = hist / len(y)
